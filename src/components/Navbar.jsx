...
const Navbar = () => {
  ...
  const handleLogout = async () => {
    try {
      // 1. If Clerk session exists, sign out from Clerk first
      if (isSignedIn) {
        await signOut();
      } else {
        // 2. Otherwise, hit the standard logout endpoint
        await axiosInstance.post("/logout", {});
      }

      // 3. Clear all Redux state
      dispatch(removeUser());
      dispatch(removeFeed());
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        {user && user.loginUser ? (
          ...
          <div className="flex items-center gap-4">
            <ThemeToggle />
            {/* Clerk sign-in — Google / OAuth */}
            <Link to="/clerk-login">
              <Button variant="ghost" className="font-semibold text-violet-600 hover:text-violet-700 hover:bg-violet-50 gap-1">
